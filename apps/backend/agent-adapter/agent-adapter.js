...
const server = app.listen(SERVER.PORT, SERVER.HOST, () => {
  logger.info(`BiteBase Agent Adapter running on http://${SERVER.HOST}:${SERVER.PORT}`);
  logger.info(`Health check: http://${SERVER.HOST}:${SERVER.PORT}/health`);
  logger.info(`Research API: http://${SERVER.HOST}:${SERVER.PORT}/api/research`);
  logger.info(`Restaurants API: http://${SERVER.HOST}:${SERVER.PORT}/api/restaurants`);
  logger.info(`Market Analysis API: http://${SERVER.HOST}:${SERVER.PORT}/api/analyze`);
  logger.info(`Geocoding API: http://${SERVER.HOST}:${SERVER.PORT}/api/geocode`);
});
