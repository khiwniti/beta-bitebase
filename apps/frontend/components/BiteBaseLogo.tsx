...
export default function BiteBaseLogo({
  ...
  const logoContent = (
    <div className={`flex items-center ${animated ? 'transition-all duration-300 hover:scale-105' : ''} ${className}`}>
      <div className="relative flex-shrink-0">
        {!imageError ? (
          ...
          <div
            ...
            style={{ width: `${iconSize}px`, height: `${iconSize}px`, fontSize: `${iconSize * 0.4}px` }}
          >
            B
          </div>
        )}
  ...
  if (clickable && href) {
    return (
      <Link
        ...
        aria-label="BiteBase Intelligence - Home"
      >
        {logoContent}
      </Link>
    )
