...
import { MarketingResearchActions } from '../marketing-research/MarketingResearchActions';
import MarketingResearchVisualizer from '../marketing-research/MarketingResearchVisualizer';
...
export const UnifiedAIChat: React.FC<UnifiedAIChatProps> = ({
  ...
  const handleMarketingAction = async (action: string, parameters: Record<string, any>) => {
    ...
    const message = actionResponses[action as keyof typeof actionResponses] || 
                   'I\'ve completed the requested action. Here are the results.';
    
    // Use the existing sendMessage function which handles loading states
