...
import { Button, Textarea, Card, CardContent, CardFooter, CardHeader, CardTitle, Avatar, AvatarFallback, AvatarImage, Badge } from '@bitebase/ui';
import { Bot, Send, Trash, User, Globe, BarChart3, TrendingUp, Users, DollarSign, MapPin } from 'lucide-react';
...
const BiteBaseAIAssistant: React.FC<BiteBaseAIAssistantProps> = ({
  ...
  const [currentLanguage, setCurrentLanguage] = useState<'th' | 'en'>(defaultLanguage);
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              size="icon"
              onClick={clearChat}
              title={currentLanguage === 'th' ? 'ล้างการสนทนา' : 'Clear conversation'}
                  ...
                  >
                    {message.role === 'assistant' && message.response ? (
                      <div>
                        ...
                        {message.response.type !== 'general_help' && (
                          ...
                          >
                            ...
                            <span className="ml-1 text-xs">
                              {message.response.type.replace('_', ' ').toUpperCase()}
                            </span>
                          </Badge>
                        )}
                        ...
                        {message.response.data && message.response.type === 'sales_analysis' && (
                          <div className="mt-3 p-3 bg-white rounded border">
                            <div className="grid grid-cols-2 gap-2 text-xs">
                              <div>
                                <span className="font-medium">Revenue:</span> ฿{message.response.data.monthlyRevenue?.toLocaleString()}
                              </div>
                              <div>
                                <span className="font-medium">Customers:</span> {message.response.data.customerCount}
                              </div>
                            </div>
                          </div>
                        )}
        ...
        {error && (
          <div className="flex justify-center">
            <div className="rounded-lg px-4 py-2 bg-red-100 text-red-700 text-sm">
              {currentLanguage === 'th' ? 'เกิดข้อผิดพลาด: ' : 'Error: '}{error}
            </div>
          </div>
        )}
